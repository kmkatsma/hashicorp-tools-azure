...
# Write base client Consul config
sudo tee /etc/consul.d/consul-client.json <<EOF
{
  "server": false
}
EOF
...
# Write base client Nomad config
...
###############################
sudo mkdir -p /tmp/nomad/init/systemd/
sudo tee /tmp/nomad/init/systemd/nomad.service <<'EOF'
[Unit]
...
SYSTEMD_DIR="/lib/systemd/system"
echo "Installing Nomad systemd service for Ubuntu"
sudo cp /tmp/nomad/init/systemd/nomad.service $${SYSTEMD_DIR}
sudo chmod 0664 $${SYSTEMD_DIR}/nomad.service

sudo systemctl enable nomad
sudo systemctl start nomad

echo "Completed Configuration of Nomad Node."
